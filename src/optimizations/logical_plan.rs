...
use crate::datatypes::stream_item::StreamItem;
...
impl LogicPlan {
    ...
    pub(crate) fn get_schema(&self) -> Schema {
        match self {
            LogicPlan::TableScan {
                ...
                projections,
                ..
            } => schema.clone().expect("Schema not found. You should set the schema after the conversion to OptStream.").with_projections(projections),
    ...
    pub(crate) fn infer_schema(&mut self, has_header: bool) {
        match self {
            LogicPlan::TableScan { path, schema, .. } => {
                *schema = Some(Schema::infer_from_file(path.clone(), has_header))
            }
            LogicPlan::Filter { input, .. } => input.infer_schema(has_header),
            LogicPlan::Select { input, .. } => input.infer_schema(has_header),
            LogicPlan::Shuffle { input } => input.infer_schema(has_header),
            LogicPlan::GroupBy { input, .. } => input.infer_schema(has_header),
            LogicPlan::DropKey { input } => input.infer_schema(has_header),
            LogicPlan::CollectVec { input } => input.infer_schema(has_header),
            LogicPlan::DropColumns { input, .. } => input.infer_schema(has_header),
            LogicPlan::Join {
                input_left,
                input_right,
                ..
            } => {
                input_left.infer_schema(has_header);
                input_right.infer_schema(has_header);
            }
            LogicPlan::UpStream { .. } => {
                panic!("Cannot infer schema from UpStream, set it manually.")
            }
            LogicPlan::Mean { input, .. } => input.infer_schema(has_header),
            LogicPlan::GroupbySelect { input, .. } => input.infer_schema(has_header),
        }
    }
}
