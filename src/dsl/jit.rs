...
use crate::datatypes::{schema::Schema, NoirType, NoirTypeKind};
...
impl JitCompiler {
    pub fn compile(&mut self, input_expr: &Expr, schema: &Schema) -> Result<*const u8, String> {
        let params = schema.types().cloned().collect();
...
impl<'a> ExprTranslator<'a> {
        ...
        match expr {
            Expr::NthColumn(n) => {
                ...
                let item_type = match self.params[*n] {
                    NoirTypeKind::Int => types::I32,
                    NoirTypeKind::Float => types::F32,
            ...
            Expr::Literal(v) => match v {
                ...
                NoirType::Float(v) => (self.builder.ins().f32const(*v), NoirTypeKind::Float),
                NoirType::Bool(v) => (
                    self.builder
                        .ins()
                ...
                NoirType::None() => (self.builder.ins().f32const(f32::NAN), NoirTypeKind::Float),
                ...
                match op {
                    BinaryOp::Plus => {
                        ...
                        let val = match values_type {
                            NoirTypeKind::Int => self.builder.ins().iadd(left_val, right_val),
                            NoirTypeKind::Float => self.builder.ins().fadd(left_val, right_val),
                    ...
                    BinaryOp::Minus => {
                        let (left_val, right_val, values_type) = self
                            .ensure_compatible_types(left_val, right_val, left_type, right_type);
                        let val = match values_type {
                            NoirTypeKind::Int => self.builder.ins().isub(left_val, right_val),
                            NoirTypeKind::Float => self.builder.ins().fsub(left_val, right_val),
                    ...
                    BinaryOp::Multiply => {
                        let (left_val, right_val, values_type) = self
                            .ensure_compatible_types(left_val, right_val, left_type, right_type);
                        let val = match values_type {
                            NoirTypeKind::Int => self.builder.ins().imul(left_val, right_val),
                            NoirTypeKind::Float => self.builder.ins().fmul(left_val, right_val),
                    ...
                    BinaryOp::Divide => {
                        let (left_val, right_val, values_type) = self
                            .ensure_compatible_types(left_val, right_val, left_type, right_type);
                        let val = match values_type {
                            NoirTypeKind::Int => self.builder.ins().sdiv(left_val, right_val),
                            NoirTypeKind::Float => self.builder.ins().fdiv(left_val, right_val),
                    ...
                    BinaryOp::Mod => {
                        let val = match (left_type, right_type) {
                            (NoirTypeKind::Int, NoirTypeKind::Int) => {
                                self.builder.ins().srem(left_val, right_val)
                            }
                            (NoirTypeKind::Float, NoirTypeKind::Int) => {
                                let left_val =
                    ...
                    BinaryOp::Gt => {
                        let (left_val, right_val, values_type) = self
                            .ensure_compatible_types(left_val, right_val, left_type, right_type);
                        let val = match values_type {
                            NoirTypeKind::Int => self.handle_int_comparison(
                                left_val,
                                right_val,
                                IntCC::SignedGreaterThan,
                            ),
                            NoirTypeKind::Float => self.handle_float_comparison(
                                left_val,
                                right_val,
                                FloatCC::GreaterThan,
                            ),
                ...
                match op {
                    UnaryOp::Floor => {
                        let val = match val_type {
                            NoirTypeKind::Float => self.builder.ins().fcvt_to_sint(types::I32, val),
                            NoirTypeKind::Int => val,
                            e => panic!("Error during jit compiling, cannot floor {:?}", e),
                        };
                        (val, NoirTypeKind::Int)
                    }
                    _ => todo!(),
                }
            }
    ...
    fn ensure_compatible_types(
        ...
        match (left_type, right_type) {
            (NoirTypeKind::Int, NoirTypeKind::Int) => (left_val, right_val, left_type),
            (NoirTypeKind::Float, NoirTypeKind::Float) => (left_val, right_val, left_type),
            (NoirTypeKind::Bool, NoirTypeKind::Bool) => (left_val, right_val, left_type),
            (NoirTypeKind::Int, NoirTypeKind::Float) => {
                let left_val = self.builder.ins().fcvt_to_sint(types::F32, left_val);
                (left_val, right_val, NoirTypeKind::Float)
            }
            (NoirTypeKind::Int, NoirTypeKind::Bool) => (left_val, right_val, NoirTypeKind::Int),
            (NoirTypeKind::Float, NoirTypeKind::Int) => {
                let right_val = self.builder.ins().fcvt_to_sint(types::F32, right_val);
                (left_val, right_val, NoirTypeKind::Float)
            }
            (NoirTypeKind::Float, NoirTypeKind::Bool) => {
                let right_val = self.builder.ins().fcvt_to_sint(types::F32, right_val);
                (left_val, right_val, NoirTypeKind::Float)
            }
            (NoirTypeKind::Bool, NoirTypeKind::Int) => (left_val, right_val, NoirTypeKind::Int),
            (NoirTypeKind::Bool, NoirTypeKind::Float) => {
                let left_val = self.builder.ins().fcvt_to_sint(types::F32, left_val);
                (left_val, right_val, NoirTypeKind::Float)
            }
            e => panic!(
...
pub mod test {
    ...
    use crate::{
        datatypes::{schema::Schema, NoirType, NoirTypeKind},
        dsl::{expressions::col, jit::JitCompiler},
    };
    ...
    fn test_ensure_compatible_types() {
        ...
        let mut translator = ExprTranslator {
            params: vec![NoirTypeKind::Int, NoirTypeKind::Float],
        ...
        assert_eq!(res, NoirTypeKind::Int);
        ...
        assert_eq!(res, NoirTypeKind::Float);
    ...
    fn test_jit_compiler() {
        let schema = Schema::from_mapping(vec![
            ("col0", NoirTypeKind::Int),
            ("col1", NoirTypeKind::Int),
        ]);
        ...
        let item = [NoirType::Int(1), NoirType::Int(2)];
        let mut result = NoirType::None();
        compiled_expr(item.as_ptr(), &mut result);
        assert_eq!(result, NoirType::Int(3));
    }
}
