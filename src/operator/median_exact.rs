...
impl<Out, PreviousOperators, NewOut, F> Operator for MedianExact<Out, PreviousOperators, NewOut, F>
where
    Out: ExchangeData,
    NewOut: ExchangeData + Ord + Add<Output = NewOut> + Div<f32, Output = NewOut>,
    PreviousOperators: Operator<Out = Out>,
    ...
    Op: Operator<Out = D> + 'static,
{
    ...
    /// ```
    pub fn median<F, I>(self, get_value: F) -> Stream<impl Operator<Out = I>>
    where
        D: ExchangeData,
...
where
    PreviousOperator: Operator<Out = NoirData>,
{
...
impl<PreviousOperators> Operator for MedianExactNoirData<PreviousOperators>
where
    PreviousOperators: Operator<Out = NoirData>,
{
    type Out = NoirData;
    ...
    Op: Operator<Out = NoirData> + 'static,
{
    ...
    pub fn quantile_exact(
        ...
        quantile: f32,
        skip_nan: bool,
    ) -> Stream<MedianExactNoirData<SimpleStartOperator<NoirData>>> {
        self.replication(Replication::One)
