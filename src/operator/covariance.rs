...
{
    pub fn covariance(self, columns: [usize; 2]) -> Stream<NoirData, impl Operator<NoirData>> { 

        let (state, stream) = self.retain_columns(columns.to_vec()).shuffle()
        .iterate(2, FillState::default() , 
        move |s, state| {
            s.map(move |v| {
