    ...
    Op: Operator<Out = I> + 'static,
{
    ...
    /// ```
    pub fn mean<O, F>(self, get_value: F) -> Stream<impl Operator<Out = O>>
    where
    ...
    Op: Operator<Out = NoirData> + 'static,
{
    ...
    /// ```
    pub fn mean_noir_data(self, skip_na: bool) -> Stream<impl Operator<Out = NoirData>> {
        self.fold_assoc(
