...
mod tests {
    ...
    use metrics::{Counter, CounterFn, Key, KeyName, Metadata, Recorder, Unit, SharedString};
    ...
    impl Recorder for MockRecorder {
        ...
        fn register_counter(&self, key: &Key, _metadata: &Metadata<'_>) -> Counter {
            ...
            let mut labels = key.labels().map(|l| format!("{}={}", l.key(), l.value())).collect::<Vec<_>>();
            labels.sort();
            if !labels.is_empty() {
                full_name.push_str(";");
    ...
    fn test_metrics_recorded() {
        ...
        let has_items_in = counters_guard.keys().any(|k| k.starts_with("renoir_items_in"));
        let has_items_out = counters_guard.keys().any(|k| k.starts_with("renoir_items_out"));
    ...
    fn test_custom_resolution_env() {
        ...
        println!("Recorded counters with 50s resolution: {:?}", *counters_guard);
        assert!(has_items_in, "Should have logged renoir_items_in metric on drop");
        assert!(has_items_out, "Should have logged renoir_items_out metric on drop");
    }
}
